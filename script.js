...
const CONFIG = {
  owner: "capimmm",
  repo: "capimm/bio",   // <- troque pelo nome do repositório
  branch: "main",
...
async function setupMedia(){
  ...
  // garante que vídeo e música comecem exatamente juntos: como o vídeo
  // pode já estar rolando (mudo) antes do navegador liberar o áudio,
  // realinhamos o vídeo pro início assim que a música tocar de verdade
  // pela primeira vez.
  let audioAligned = !hasAudio;
  if(hasAudio && hasVideo){
    bgAudio.addEventListener("play", () => {
      if(!audioAligned){
        bgVideo.currentTime = bgAudio.currentTime;
        audioAligned = true;
      }
    });
  }
  ...
  //      depois de mais 5s parada, o QR cresce e o texto some. ----
  let idleTimer = null;
  let donateGrowTimer = null;

  function expandHotbar(){
    clearTimeout(idleTimer);
    clearTimeout(donateGrowTimer);
    hotbar.classList.remove("is-collapsed");
    hotbarDonate.hidden = true;
    hotbarDonate.classList.remove("is-compact");
  ...
  function collapseHotbar(){
    ...
    donateGrowTimer = setTimeout(() => {
      hotbarDonate.classList.add("is-compact");
    }, 5000);
  }
